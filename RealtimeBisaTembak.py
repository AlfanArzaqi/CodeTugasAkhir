...
def RecShoot ():
    ...
    msgrcv2 = (my_socket.recv(1024))
    # msgrcv2.decode(encoding='UTF-8',errors='strict')
    # df = pd.DataFrame(msgrcv2)
    # df.to_csv('sample_data.csv', index=False)
    # msgrcv2 = pd.read_csv('sample_data.csv', header= 0, encoding= 'unicode_escape')
...
if __name__ == "__main__":
    ...
    with mp_hands.Hands(
        ...
        while cap.isOpened():
            ...
            # t2 = threading.Thread(target=ExtrakHand(hands, image) , name='t2')
            t1.start()
            # t2.start()

            # Program Tertembak
            # msgrcv = my_socket.recv(1024).decode()
            # print(msgrcv,end='')
