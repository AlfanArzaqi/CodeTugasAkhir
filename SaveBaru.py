...
def ExtrakHand (hands, image):
    ...
    mergeimage = []
    handsType = []
    kiriImage = np.zeros((128,128,3), dtype="uint8")
    kananImage = np.zeros((128,128,3), dtype="uint8")
    ...
    if results.multi_hand_landmarks:
      for hand_handedness in results.multi_handedness :
        ...
        for hand_landmarks in results.multi_hand_landmarks:
          ...
          mergeimage = np.hstack((kiriImage, kananImage))
...
sDirektoriData = "D:\DATA\SEMESTER 8\TUGASAKHIR\Mentah\\"+sNamaDirektori 
